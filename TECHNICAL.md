...
## Configuration release registry

`src/releases.rs` implements portable source bundles, dependency closure, immutable revision identity, transactional publication and compare-and-swap activation. `migrations/0003_releases.sql` stores revisions, drafts, releases, active pointers and activation audit. `Platform::resolve` is shared by local files, bundle validation and registry execution. Production submissions read one complete active bundle before constructing the job snapshot; workers never query mutable prompt state. Follow-ups inherit the root snapshot.

Configuration API (authenticated): `GET /api/configuration`, `PUT /api/configuration/drafts/{uuid}` with expected draft revision, `POST /api/configuration/validate`, `POST /api/configuration/prompts`, `POST /api/configuration/releases`, `GET /api/configuration/releases/{uuid}/export`, `POST /api/configuration/releases/{uuid}/activate` with expected activation generation and reason, and `POST /api/configuration/drafts/{uuid}/test`. Publishing uses the exported base generation to reject stale changes. Editor and publisher permissions are distinct global roles. Draft tests also require repository operator access and fixture-only execution. There is no delete API for historical revisions/releases.

The registry is the default source; `FACTORY_CONFIGURATION_SOURCE=files` explicitly opts into development snapshots and disables mutations. Existing jobs deserialize without release fields. A one-time atomic bootstrap imports bundled definitions only into an empty registry. Runtime profile settings remain deployment-managed and are validated again at activation/submission; receipts pin the actual runtime configuration used. Bundle hashes exclude export metadata and canonicalize YAML structure.


The Design workspace separates workflow design from operations and provides a grouped prompt library. `0004_design_workspace.sql` adds per-workflow release numbers, draft lifecycle state, and durable fixture-test records keyed to the candidate digest. `POST /api/configuration/prepare` validates and assigns immutable numeric revisions using parsed task references. `POST /api/configuration/prompts/revise` publishes a prompt and creates explicitly selected consumer drafts in one transaction, checking active generations and existing drafts. `POST /api/configuration/drafts/{uuid}/discard` uses expected draft revision. Release publication accepts `draft_id`, `draft_revision`, and `activate`: the saved candidate, release, optional active pointer, audit, and draft closure are checked/committed atomically. These operations retain the existing editor/publisher authorization boundaries.
